...
from . import abcvna
from . import rs_zvl_scpi


class ZVL(abcvna.VNA):
    """
    Class for modern Rohde&Schwarz ZVL Vector Network Analyzers
    """
    ...
    def __init__(self, address, **kwargs):
        ...
        self.scpi = rs_zvl_scpi.SCPI(self.resource)
    ...
    def measurement_mode(self):
        ...
        try:
            channel = self.scpi.query_measurement_mode()
    ...
    def measurement_mode(self, channel):
        ...
        if channel in self.channel_list:
            self.scpi.set_measurement_mode(channel)
        else:
            print('Channel %i not in list of channels. Create channel first'
                  % channel)
        return self.scpi.query_measurement_mode()
    ...
    def get_snp_network(self, ports, **kwargs):
        if "sweep" in kwargs.keys():
            warnings.warn("Sweep function not yet implemented for ZVL")
